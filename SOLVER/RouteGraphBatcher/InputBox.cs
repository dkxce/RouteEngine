...
namespace System.Windows.Forms
{
    public class InputBox
    {
        ...
        private DialogResult ShowRegex(string testerText, bool allow_new)
        {
            Form form = new InputBoxForm();
            form.ShowInTaskbar = pShowInTaskBar;
            Label label = new Label();
        ...
        private static Label query_info_btm = null;
        private static EventBtnHandler query_info_evn = null;
        public delegate void EventBtnHandler(object sender, ref string infoText, ref string bottomText, ref bool close);
        ...
        public static DialogResult QueryInfoBox(string title, string topText, string bottomText, ref string mainText)
        {
            return QueryInfoBox(title, topText, bottomText, ref mainText, true, false, MessageBoxButtons.OK, true);
        }
        public static DialogResult QueryInfoBox(string title, string topText, string bottomText, ref string mainText, bool readOnly, bool allowNewLoadSave, MessageBoxButtons buttons, bool closeButton)
        {
            return QueryInfoBox(title, topText, bottomText, ref mainText, true, false, MessageBoxButtons.OK, true, null, null);
        }
        public static DialogResult QueryInfoBox(string title, string topText, string bottomText, ref string mainText, bool readOnly, bool allowNewLoadSave, MessageBoxButtons buttons, bool closeButton, string[] buttonsText, EventBtnHandler onclick)
        {
            Form form = closeButton ? new Form() : new InputBoxForm();
            form.DialogResult = DialogResult.Cancel;
            form.ShowInTaskbar = pShowInTaskBar;
            ...
            for (int i = 0; i < btnCount; i++)
            {
                ...
                if (i == 0)
                {
                    ...
                    if (buttons == MessageBoxButtons.YesNo) { Buttons[i].Text = pOk_Yes; Buttons[i].DialogResult = DialogResult.Yes; };
                    if (buttons == MessageBoxButtons.RetryCancel) { Buttons[i].Text = pOk_Retry; Buttons[i].DialogResult = DialogResult.Retry; };
                    if ((buttonsText != null) && (buttonsText.Length > 0)) Buttons[i].Text = buttonsText[0];
                };
                if (i == 1)
                {
                    if (buttons == MessageBoxButtons.OKCancel) { Buttons[i].Text = pCancel_Text; Buttons[i].DialogResult = DialogResult.Cancel; };
                    ...
                    if (buttons == MessageBoxButtons.YesNo) { Buttons[i].Text = pOk_No; Buttons[i].DialogResult = DialogResult.No; };
                    if (buttons == MessageBoxButtons.RetryCancel) { Buttons[i].Text = pCancel_Text; Buttons[i].DialogResult = DialogResult.Cancel; };
                    if ((buttonsText != null) && (buttonsText.Length > 1)) Buttons[i].Text = buttonsText[1];
                };
                if (i == 2)
                {
                    if (buttons == MessageBoxButtons.AbortRetryIgnore) { Buttons[i].Text = pOk_Ignore; Buttons[i].DialogResult = DialogResult.Ignore; };
                    if (buttons == MessageBoxButtons.YesNoCancel) { Buttons[i].Text = pCancel_Text; Buttons[i].DialogResult = DialogResult.Cancel; };
                    if ((buttonsText != null) && (buttonsText.Length > 2)) Buttons[i].Text = buttonsText[2];
                };
                Buttons[i].BackColor = System.Drawing.SystemColors.Control;
                Buttons[i].Location = new System.Drawing.Point(9 + 81 * i, 10);
                Buttons[i].Size = new System.Drawing.Size(75, 23);
                if (onclick != null)
                {
                    ...
                    query_info_evn = onclick;
                    Buttons[i].Click += new EventHandler(QIB_Click);
                };
            };
            panel3.Controls.AddRange(Buttons);
        ...
        static void QIB_Click(object sender, EventArgs e)
        {
            if ((query_info_evn != null) && (query_info_box != null) && (query_info_btm != null))
            {                
                string text = query_info_box.Text;
                string botm = query_info_btm.Text;
                bool close = false;
                query_info_evn(sender, ref text, ref botm, ref close);
                query_info_box.Text = text;
                query_info_btm.Text = botm;
                if (close) ((Form)((Control)sender).Parent.Parent.Parent).Close();
            };
        }
