...
namespace dkxce.PreSolvedRoutes
{
    /// <summary>
    ...
    public class RMMatrix
    {
        ...
        */
        ...
        /// </summary>
        private const uint recordLength = 4 + 4 + 4 + 4; // prev_node + cost + dist + time
        ...
        public void AddWay(uint x, uint y, Single cost, Single dist, Single time)
        {
        ...
        public void Solve()
        {
            ...
            for (uint k = 1; k <= size; k++)
            {
                ...
                {
                    float ik = GetCost(i, k);
                    for (uint j = 1; j <= size; j++)
                    {
                        float ikj = ik + GetCost(k, j);
                        if (ikj < GetCost(i, j))
                        {
                            SetCost(i, j, ikj);
                            SetPrev(i, j, k);
        ...
        public uint[] GetRouteWay(uint x, uint y)
        {
            ...
            while (((intermediate = GetPrev(x,intermediate)) > 0) && (intermediate != x))
            ...
            for (uint x = 1; x <= size; x++)
            {
                ...
                for (uint y = 1; y <= size; y++)
                    if (GetCost(x, y) > maxValue)
                        ...
                        sw.Write(GetPrev(x, y).ToString() + "(" + GetCost(x, y).ToString("0.000").Replace(",", ".") + ") ;");
                sw.WriteLine();
            };
            sw.WriteLine();
            sw.Write("DIST;");
